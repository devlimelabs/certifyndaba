...
export const requestAcceptedEmail = onDocumentUpdated(`companies/{companyID}/requests/{requestID}`, async event => {
  ...
  if (oldStatus !== 'verified' && newStatus === 'verified') {
    ...
    db.collection('mail').add({
        ...
        subject: `Your Connection Request was Accepted on CertifyndABA!`,
        html: `<h1>Your Connection Request was Accepted on CertifyndABA!</h1>
        ...
        text: `
          You're Connection Request has been Accepted for:

          ${after?.title}
          ${after?.salary} - ${after?.city}, ${after?.state}

          To view the contact details of the request candidate, go to your Request Dashboard:
